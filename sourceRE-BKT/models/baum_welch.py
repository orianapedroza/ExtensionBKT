...
class BaumWelchREBKT:
    ...
    Se enfoca en una única emoción continua (Concentración) y optimiza
    las matrices de transición y observación.
    """
    ...
    def __init__(self, max_iterations: int = 100, tolerance: float = 1e-4):
        ...
        self.P_IC0, self.P_CC0 = None, None  # Emisión: Estado No Aprendido
        self.P_IC1, self.P_CC1 = None, None  # Emisión: Estado Aprendido
    ...
    def initialize_parameters(self, seed: int = 42):
        ...
        row0 = np.random.uniform(0.01, 1.0, 2)
        row0 /= row0.sum()
        self.P_IC0, self.P_CC0 = row0[0], row0[1]

        row1 = np.random.uniform(0.01, 1.0, 2)
        row1 /= row1.sum()
        self.P_IC1, self.P_CC1 = row1[0], row1[1]

    def get_params(self) -> Dict[str, float]:
        """Retorna un diccionario con los parámetros actuales del modelo."""
        return {
            'P_L0': self.P_L0, 'P_T': self.P_T,
            'P_IC0': self.P_IC0, 'P_CC0': self.P_CC0,
            'P_IC1': self.P_IC1, 'P_CC1': self.P_CC1,
        }
    ...
    def get_observation_matrix(self) -> np.ndarray:
        return np.array([
            [self.P_IC0, self.P_CC0],
            [self.P_IC1, self.P_CC1],
        ])
    ...
    def m_step(self, observations_list: List[List[Tuple[int, float]]],
               gamma_list: List[np.ndarray], xi_list: List[np.ndarray]) -> Dict[str, float]:
        ...
        row0 = np.array([new_P_IC0, new_P_CC0])
        row0 /= row0.sum()

        new_P_IC1 = np.clip(new_P_IC1, 0.01, 0.99)
        new_P_CC1 = np.clip(new_P_CC1, 0.01, 0.99)
        row1 = np.array([new_P_IC1, new_P_CC1])
        row1 /= row1.sum()

        return {
            'P_L0': new_P_L0, 'P_T': new_P_T,
            'P_IC0': row0[0], 'P_CC0': row0[1],
            'P_IC1': row1[0], 'P_CC1': row1[1],
        }

    def fit(self, observations_list: List[List[Tuple[int, float]]], verbose: bool = False):
        ...
        for iteration in range(self.max_iterations):
            ...
            new_params = self.m_step(observations_list, gamma_list, xi_list)
